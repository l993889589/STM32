...
主站模式改为 `MODBUS_APP_ROLE_MASTER`。示例主站每秒向地址 1 发送一次 FC03，请求保持寄存器 0 和 1。默认从站支持 FC01/02/03/04/05/06/0F/10/16/17。线圈 0..63 和保持寄存器 0..63 默认清零，并完整保留给远程主站读写。输入寄存器 0..7 提供协议只读诊断：芯片标识、收发计数、错误计数及 T1.5/T3.5；可使用 FC04 读取。诊断量不会覆盖主站写入的保持寄存器。
