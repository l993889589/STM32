...
static void USBD_BuildSerialNumber(uint8_t serial[13]);
...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
  ...
  USBD_BuildSerialNumber(serial);
  USBD_Desc_GetString(serial, USBD_string_framework + count, &len);
...
static void USBD_WriteHex(uint32_t value, uint8_t *output, uint8_t digits)
{
...
static void USBD_BuildSerialNumber(uint8_t serial[13])
{
  ...
  USBD_WriteHex(uid[0] + uid[2], &serial[0], 8U);
  USBD_WriteHex(uid[1], &serial[8], 4U);
  serial[12] = 0U;
}
