...
static char *finsh_prompt_custom = "leduo >";
...
static void shell_push_history(struct finsh_shell *shell)
{
    if (g_shell.line_position != 0)
    {
        ...
        if (g_shell.history_count >= FINSH_HISTORY_LINES)
        {
            ...
            if (mmemcmp(&g_shell.cmd_history[FINSH_HISTORY_LINES - 1], g_shell.line, FINSH_CMD_SIZE))
            {
                /* move history */
                int index;
                for (index = 0; index < FINSH_HISTORY_LINES - 1; index ++)
                {
                    mmemcpy(&g_shell.cmd_history[index][0],
                           &g_shell.cmd_history[index + 1][0], FINSH_CMD_SIZE);
                }
                mmemset(&g_shell.cmd_history[index][0], 0, FINSH_CMD_SIZE);
                mmemcpy(&g_shell.cmd_history[index][0], g_shell.line, g_shell.line_position);
        ...
        {
            /* if current cmd is same as last cmd, don't push */
            if (g_shell.history_count == 0 || mmemcmp(&g_shell.cmd_history[g_shell.history_count - 1], g_shell.line, FINSH_CMD_SIZE))
            {
                g_shell.current_history = g_shell.history_count;
                mmemset(&g_shell.cmd_history[g_shell.history_count][0], 0, FINSH_CMD_SIZE);
                mmemcpy(&g_shell.cmd_history[g_shell.history_count][0], g_shell.line, g_shell.line_position);
    ...
    while (1)
    {
        ...
        else if (g_shell.stat == WAIT_FUNC_KEY)
        {
            ...
            if (ch == 0x41) /* up key */
            {
                ...
                mmemcpy(g_shell.line, &g_shell.cmd_history[g_shell.current_history][0],
                       FINSH_CMD_SIZE);
                g_shell.line_curpos = g_shell.line_position = mstrlen(g_shell.line);
        ...
        else if (ch == 0x7f || ch == 0x08)
        {
            ...
            if (g_shell.line_position > g_shell.line_curpos)
            {
                ...
                mmemmove(&g_shell.line[g_shell.line_curpos],
        ...
        if (ch == '\r' || ch == '\n')
        {
            ...
            else
            {
                ...
                g_shell.line[g_shell.line_position] = ';';

                if (g_shell.line_position != 0) finsh_run_line(&g_shell.parser, g_shell.line);
                else
                    if (g_shell.echo_mode) putstr("\n");
            }
#endif
            putstr(FINSH_PROMPT);
            mmemset(g_shell.line, 0, sizeof(g_shell.line));
        ...
        if (g_shell.line_curpos < g_shell.line_position)
        {
            ...
            mmemmove(&g_shell.line[g_shell.line_curpos + 1],
