/**
 * @file bsp_spi.c
 * @brief Shared SPI1 bus initialization, transfer and ownership control.
 */
...
*********************************************************************************************************
*/
/** @brief Apply SPI clock and phase parameters when they have changed. */
...
{
	(void)bsp_spiTransferTimeout(100U);
}

/** @brief Transfer the prepared SPI buffers with a bounded timeout. */
uint8_t bsp_spiTransferTimeout(uint32_t timeout_ms)
{
	uint32_t start;

	if (g_spiLen == 0U || g_spiLen > SPI_BUFFER_SIZE || timeout_ms == 0U)
		return 0U;

#ifdef USE_SPI_DMA
	wTransferState = TRANSFER_WAIT;
	if(HAL_SPI_TransmitReceive_DMA(&hspi, g_spiTxBuf, g_spiRxBuf, g_spiLen) != HAL_OK)
		return 0U;
	start = HAL_GetTick();
	...
	return wTransferState == TRANSFER_COMPLETE ? 1U : 0U;
#endif

#ifdef USE_SPI_INT
	wTransferState = TRANSFER_WAIT;
	if(HAL_SPI_TransmitReceive_IT(&hspi, g_spiTxBuf, g_spiRxBuf, g_spiLen) != HAL_OK)
		return 0U;
	start = HAL_GetTick();
	while (wTransferState == TRANSFER_WAIT)
	{
...
#ifdef USE_SPI_POLL
	return HAL_SPI_TransmitReceive(&hspi, g_spiTxBuf, g_spiRxBuf, g_spiLen,
...
/** @brief Mark the current SPI transfer failed after a HAL error callback. */
...
*/
#ifdef USE_SPI_INT
	/** @brief Forward the configured SPI interrupt to the HAL driver. */
...
#ifdef USE_SPI_DMA
	/** @brief Forward the configured SPI RX DMA interrupt to the HAL driver. */
	void SPIx_DMA_RX_IRQHandler(void)
	{
		HAL_DMA_IRQHandler(hspi.hdmarx);
	}

	/** @brief Forward the configured SPI TX DMA interrupt to the HAL driver. */
