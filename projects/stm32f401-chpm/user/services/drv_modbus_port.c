...
static ULONG modbus_gap_ticks(uint32_t remaining_cycles)
{
    ...
    if(bsp_dwt_frequency_hz() == 0U)
        return 1U;
    ticks = ((uint64_t)remaining_cycles * TX_TIMER_TICKS_PER_SECOND +
             bsp_dwt_frequency_hz() - 1U) /
            bsp_dwt_frequency_hz();
...
{
    (void)context;
    ld_modbus_rtu_framer_on_error(&modbus_framer, bsp_dwt_get_cycles());
    modbus_event_byte_index = 0U;
...
bsp_status_t drv_modbus_port_init(uint32_t baud_rate)
{
    ...
    if(baud_rate == 0U || bsp_dwt_frequency_hz() == 0U ||
