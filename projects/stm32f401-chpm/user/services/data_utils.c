...
void SetBit(void *data, uint8_t bit_position, uint8_t value, size_t size) {
    if (bit_position >= size * 8) return; // 检查位宽

    switch (size) {
        case sizeof(uint8_t):
            if (value)
                *(uint8_t *)data |= (1 << bit_position);
            else
                *(uint8_t *)data &= ~(1 << bit_position);
            break;
...
uint8_t GetBit(void *data, uint8_t bit_position, size_t size) {
    if (bit_position >= size * 8) return 0; // 检查位宽

    switch (size) {
        case sizeof(uint8_t):
            return (*(uint8_t *)data & (1 << bit_position)) ? 1 : 0;

        case sizeof(uint16_t):
            return (*(uint16_t *)data & (1 << bit_position)) ? 1 : 0;

        case sizeof(uint32_t):
            return (*(uint32_t *)data & (1 << bit_position)) ? 1 : 0;
...
int main() {
    ...
    // 设置第5位为1
    SetBit(&warning, 5, 1, sizeof(warning));
    PrintBinary(&warning, sizeof(warning));

    // 清除第5位为0
    SetBit(&warning, 5, 0, sizeof(warning));
    PrintBinary(&warning, sizeof(warning));

    // 获取第5位的状态
    printf("Bit 5 state: %d\n", GetBit(&warning, 5, sizeof(warning)));
