...
/* RTC-stamped, power-loss-safe circular black-box event journal. */
#define BLACKBOX_EXT_FLASH_ADDR             OTA_EXT_DIAGNOSTIC_ADDR
#define BLACKBOX_EXT_FLASH_SIZE             OTA_EXT_DIAGNOSTIC_SIZE
#define BLACKBOX_EXT_SECTOR_SIZE            OTA_EXT_SECTOR_SIZE
...
#define OTA_EXT_FACTORY_IDENTITY_ADDR      0x00FFF000UL
#define OTA_EXT_FACTORY_IDENTITY_SIZE      OTA_EXT_SECTOR_SIZE
...
#if (BLACKBOX_EXT_FLASH_ADDR % BLACKBOX_EXT_SECTOR_SIZE) != 0UL || \
    (BLACKBOX_EXT_FLASH_SIZE % BLACKBOX_EXT_SECTOR_SIZE) != 0UL || \
    (BLACKBOX_EXT_FLASH_ADDR + BLACKBOX_EXT_FLASH_SIZE) > UI_ASSET_SLOT_A_ADDR
#error "Black-box journal must stay sector-aligned inside the diagnostic region"
#endif
...
#if (OTA_EXT_FACTORY_IDENTITY_ADDR + OTA_EXT_FACTORY_IDENTITY_SIZE) != OTA_EXT_FLASH_SIZE
#error "Factory identity sector must occupy the final external-flash sector"
#endif
