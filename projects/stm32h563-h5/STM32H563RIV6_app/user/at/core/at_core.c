...
/** @brief Wipe a transient command buffer without allowing dead-store removal. */
static void at_secure_zero(void *data, size_t length)
{
...
static void at_handle_line(at_client_t *at, char *line)
{
    ...
    /* URCs are never copied into the active command response/capture. */
    ...
    if(at->result == AT_RESULT_OVERFLOW)
        return;

    if(at_is_final_error(line))
        at->result = AT_RESULT_ERROR;
    else if(at->success_prefix != NULL)
    {
        success_length = strlen(at->success_prefix);
        if(success_length != 0U &&
           strncmp(line, at->success_prefix, success_length) == 0)
        {
            at->result = AT_RESULT_OK;
        }
    }
    else if(at_is_final_ok(line))
    {
        at->result = AT_RESULT_OK;
    }
}
...
/** @brief Arm a response-only wait without transmitting a new command. */
int at_client_begin_wait(at_client_t *at,
                         const char *success_prefix,
                         uint32_t now_ms,
                         uint32_t timeout_ms)
{
    if(at == NULL || success_prefix == NULL || success_prefix[0] == '\0')
        return -1;
    if(at->result == AT_RESULT_PENDING)
        return -2;
    ...
    at_client_raw_cancel(at);
    at->active_cmd = NULL;
    at->success_prefix = success_prefix;
    at->deadline_ms = now_ms + timeout_ms;
    at->command_epoch++;
    at->result = AT_RESULT_PENDING;
    return 0;
...
/** @brief Select the line prefix that completes the current command. */
...
/** @brief Fail the active transaction after a discontinuous receive stream. */
void at_client_transport_error(at_client_t *at)
{
    ...
    at_client_raw_cancel(at);
    at->line_len = 0U;
    at->line_discarding = 1U;
    at->result = AT_RESULT_TRANSPORT_ERROR;
}
...
void at_client_input(at_client_t *at, const uint8_t *data, uint32_t len)
{
    ...
    for(uint32_t i = 0U; i < len; i++)
    {
        ...
        if(at->line_discarding != 0U)
        {
            if(ch == '\n')
                at->line_discarding = 0U;
            continue;
        }
