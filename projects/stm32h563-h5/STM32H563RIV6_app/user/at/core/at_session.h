...
#ifndef AT_SESSION_CAPTURE_MAX_LEN
#define AT_SESSION_CAPTURE_MAX_LEN 4096U
#endif
...
/** @brief Monotonic diagnostics for line-oriented AT transactions. */
typedef struct
{
    uint32_t attempts;
    uint32_t successes;
    ...
    uint32_t resync_failures;
    uint32_t poll_limit_exits;
} at_session_command_diag_t;
...
typedef struct
{
    ...
    void *log_arg;
    void *poll_arg;
    char capture[AT_SESSION_CAPTURE_MAX_LEN];
    ...
    uint32_t rx_epoch;
    uint32_t transport_error_epoch;
    at_session_binary_diag_t binary_diag;
    at_session_command_diag_t command_diag;
} at_session_t;
...
/**
 * @brief Authorize a new AT transaction after the caller reset the transport peer.
 * @param session Session whose parser and pending transaction state are discarded.
 * @note Call only after a hardware/module reset or another recovery operation that
 *       guarantees responses from the previous command can no longer arrive.
 *       Task context and a valid monotonic now_ms callback are required.
 * @return false if the clock/transport never demonstrates the required quiet period.
 */
bool at_session_recover_after_transport_reset(at_session_t *session);
/** @brief Return true while ambiguous late responses make the session unsafe to reuse. */
...
bool at_session_cmd_expect(at_session_t *session,
                           ...
                           uint8_t retries);
/**
 * @brief Capture a multi-line command response until the receive burst is idle.
 * @note Intended for commands such as W800 AT+WSCAN whose response is terminated
 *       by an empty line that the generic AT line parser intentionally ignores.
 */
