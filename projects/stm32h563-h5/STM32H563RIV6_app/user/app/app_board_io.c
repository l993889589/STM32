/**
 * @file app_board_io.c
 * @brief Shell-facing board diagnostics composed from App services and BSP APIs.
 */
...
void app_board_get_status(app_board_status_t *status)
{
    ...
    status->wifi_provisioning_active = w800.provisioning_active;
    status->wifi_provision_attempts = w800.provision_attempts;
    status->wifi_provision_timeouts = w800.provision_timeouts;
    status->wifi_usb_rescue_state = w800.usb_rescue_state;
    status->wifi_usb_rescue_attempts = w800.usb_rescue_attempts;
