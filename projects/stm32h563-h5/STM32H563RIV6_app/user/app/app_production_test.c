                                            ...
                                            uint32_t device_id[3])
{
    ...
    if(bsp_flash_read(OTA_EXT_FACTORY_IDENTITY_ADDR,
                      (uint8_t *)&identity,
                      sizeof(identity)) &&
    ...
    identity.valid = APP_PRODUCTION_ID_VALID;
    (void)memcpy(device_id, identity.device_id, sizeof(identity.device_id));
    if(bsp_flash_erase_4k(OTA_EXT_FACTORY_IDENTITY_ADDR) &&
       bsp_flash_write(OTA_EXT_FACTORY_IDENTITY_ADDR,
                       (const uint8_t *)&identity,
                       sizeof(identity)) &&
       bsp_flash_read_verify(OTA_EXT_FACTORY_IDENTITY_ADDR,
                             (const uint8_t *)&identity,
                             sizeof(identity)))
    {
        return APP_PRODUCTION_ID_SOURCE_FLASH;
