/**
 * @file mqtt_packet.c
 * @brief Allocation-free MQTT control-packet encoder and response parser.
 */
...
uint16_t mqtt_build_subscribe_many(uint8_t *out,
                                   ...
                                   uint8_t qos)
{
    uint32_t remaining = 2U;
    uint32_t pos = 0U;

    if(!out || !topics || topic_count == 0U || qos > 2U)
        return 0U;

    for(uint8_t i = 0U; i < topic_count; i++)
    {
        if(!topics[i])
            return 0U;
        remaining += 2U + (uint32_t)strlen(topics[i]) + 1U;
    }
    ...
    for(uint8_t i = 0U; i < topic_count; i++)
    {
        pos += mqtt_write_utf8(&out[pos], topics[i]);
        out[pos++] = qos;
    }
