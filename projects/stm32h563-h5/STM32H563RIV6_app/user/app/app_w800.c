...
#define APP_W800_INPUT_FRAME_SIZE    1280U
#define APP_ENABLE_W800_LOG          0U
#define APP_W800_UART_RX_BUF_SIZE    256U
#define APP_W800_LDC_SLOT_COUNT      16U
...
static ldc_t g_ldc = LDC_CONTEXT_INITIALIZER;
static ldc_slot_t g_ldc_slots[APP_W800_LDC_SLOT_COUNT];
static uint8_t g_ldc_storage[
    APP_W800_LDC_SLOT_COUNT * APP_W800_UART_RX_BUF_SIZE];
...
static volatile uint8_t g_rx_discontinuity;
static volatile uint32_t g_rx_discontinuities;
...
static TX_MUTEX g_scan_mutex;
static uint8_t g_scan_mutex_ready;
static uint8_t g_scan_requested;
static app_w800_scan_snapshot_t g_scan_snapshot;
...
static uint8_t g_input_frame[APP_W800_INPUT_FRAME_SIZE];
...
static bool app_w800_ssid_is_valid(const char *ssid)
{
    size_t length;
    size_t index;
    ...
    if(ssid == NULL)
        return false;
    length = strlen(ssid);
    if(length == 0U || length > APP_W800_SSID_MAX_LENGTH)
        return false;
    ...
    for(index = 0U; index < length; index++)
    {
        ...
        /* An SSID is an opaque byte sequence and may contain UTF-8. Reject
         * only controls and bytes unsafe in a quoted W800 AT command.
         */
...
static bool app_w800_take_scan_request(void)
{
    bool requested = false;

    if(g_scan_mutex_ready == 0U)
        return false;
    if(tx_mutex_get(&g_scan_mutex, TX_WAIT_FOREVER) != TX_SUCCESS)
        return false;
    if(g_scan_requested != 0U)
    {
        g_scan_requested = 0U;
        g_scan_snapshot.state = APP_W800_SCAN_RUNNING;
        g_scan_snapshot.count = 0U;
        g_scan_snapshot.generation++;
        requested = true;
    }
    (void)tx_mutex_put(&g_scan_mutex);
    return requested;
}

static void app_w800_publish_scan(bool succeeded,
                                  const at_w800_access_point_t *access_points,
                                  uint8_t count)
{
    uint8_t index;

    if(g_scan_mutex_ready == 0U ||
       tx_mutex_get(&g_scan_mutex, TX_WAIT_FOREVER) != TX_SUCCESS)
        return;
    if(count > APP_W800_SCAN_MAX_RESULTS)
        count = APP_W800_SCAN_MAX_RESULTS;
    memset(g_scan_snapshot.access_points, 0,
           sizeof(g_scan_snapshot.access_points));
    if(succeeded)
    {
        for(index = 0U; index < count; index++)
        {
            (void)strncpy(g_scan_snapshot.access_points[index].ssid,
                          access_points[index].ssid,
                          sizeof(g_scan_snapshot.access_points[index].ssid) - 1U);
            g_scan_snapshot.access_points[index].rssi_dbm =
                access_points[index].rssi_dbm;
            g_scan_snapshot.access_points[index].channel =
                access_points[index].channel;
            g_scan_snapshot.access_points[index].encryption =
                access_points[index].encryption;
        }
        g_scan_snapshot.count = count;
        g_scan_snapshot.state = APP_W800_SCAN_READY;
    }
    else
    {
        g_scan_snapshot.count = 0U;
        g_scan_snapshot.state = APP_W800_SCAN_FAILED;
    }
    g_scan_snapshot.generation++;
    (void)tx_mutex_put(&g_scan_mutex);
}
...
static bool app_w800_wait_socket_rx_data(int socket_id, uint32_t timeout_ms)
{
    ...
    while(true)
    {
        elapsed_ms = (uint32_t)(app_w800_now_ms(NULL) - start_ms);
        if(elapsed_ms >= timeout_ms)
            break;
        remaining_ms = timeout_ms - elapsed_ms;
        command_timeout_ms = remaining_ms < 1000U ? remaining_ms : 1000U;

        (void)snprintf(command, sizeof(command), "AT+SKSTT=%d", socket_id);
        app_w800_clear_at_rx_state();
        if(at_session_cmd_expect(&g_at_session,
                                 command,
                                 "+OK=",
                                 command_timeout_ms,
                                 1U))
        ...
        if(elapsed_ms >= timeout_ms)
            break;
        remaining_ms = timeout_ms - elapsed_ms;
        tx_thread_sleep(app_w800_ms_to_ticks(remaining_ms < 100U ?
                                              remaining_ms : 100U));
    }
...
/** @brief Enter the short producer/consumer metadata critical section. */
static ldc_lock_state_t app_w800_ldc_lock(void *argument)
{
    (void)argument;
    return (ldc_lock_state_t)bsp_irq_lock();
}

/** @brief Restore the exact interrupt mask captured by app_w800_ldc_lock(). */
static void app_w800_ldc_unlock(void *argument, ldc_lock_state_t state)
{
    (void)argument;
    bsp_irq_unlock((bsp_irq_state_t)state);
}

/** @brief Report whether at least one complete W800 transport block is queued. */
static bool app_w800_ldc_frame_pending(void)
{
    ldc_stats_t stats;

    return (ldc_get_stats(&g_ldc, &stats) == LDC_STATUS_OK) &&
           (stats.ready_frames != 0U);
}
...
static void app_w800_drain_input(void)
{
    ...
    while(ldc_frame_read(&g_ldc,
                         g_input_frame,
                         sizeof(g_input_frame),
...
/** @brief Fail the active AT transaction and clear queued bytes after RX loss. */
static void app_w800_handle_rx_discontinuity(void)
{
    bsp_irq_state_t state;
    uint8_t handled = 0U;

    state = bsp_irq_lock();
    if(g_rx_discontinuity != 0U && ldc_reset(&g_ldc) == LDC_STATUS_OK)
    {
        g_rx_discontinuity = 0U;
        g_rx_discontinuities++;
        handled = 1U;
    }
    bsp_irq_unlock(state);

    if(handled != 0U)
    {
        at_session_transport_error(&g_at_session);
...
static void app_w800_poll_input(void *arg)
{
    ...
    app_w800_handle_rx_discontinuity();
...
static void app_w800_uart_rx(bsp_uart_port_t port,
                             ...
                             void *arg)
{
    ldc_write_result_t write_result;
    ldc_write_result_t boundary_result;

    (void)port;
    (void)arg;
    if(g_rx_discontinuity != 0U)
        return;
    if(data && length != 0U)
    {
        write_result = ldc_rx_write(&g_ldc, data, length);
        if(write_result.status != LDC_STATUS_OK ||
           write_result.accepted_bytes != (size_t)length ||
           write_result.rejected_bytes != 0U ||
           write_result.dropped_frames != 0U ||
           write_result.overflow_frames != 0U)
        {
            (void)ldc_rx_abort(&g_ldc);
            g_rx_discontinuity = 1U;
            (void)tx_semaphore_put(&g_rx_sem);
            return;
        }

        boundary_result = ldc_rx_idle(&g_ldc);
        if(boundary_result.status != LDC_STATUS_OK ||
           boundary_result.committed_frames != 1U ||
           boundary_result.dropped_frames != 0U ||
           boundary_result.overflow_frames != 0U)
        {
            (void)ldc_rx_abort(&g_ldc);
            g_rx_discontinuity = 1U;
            (void)tx_semaphore_put(&g_rx_sem);
...
/** @brief Latch a UART/DMA receive gap for task-context AT recovery. */
static void app_w800_uart_rx_error(bsp_uart_port_t port, void *arg)
{
    (void)port;
    (void)arg;
    g_rx_discontinuity = 1U;
...
UINT app_w800_init(void)
{
    ...
    status = tx_mutex_create(&g_scan_mutex,
                             "w800 scan",
                             TX_INHERIT);
    if(status != TX_SUCCESS)
        return status;
    g_scan_mutex_ready = 1U;
    ...
    ldc_config.storage = g_ldc_storage;
    ldc_config.storage_size = sizeof(g_ldc_storage);
    ldc_config.slots = g_ldc_slots;
    ldc_config.slot_count = APP_W800_LDC_SLOT_COUNT;
    ldc_config.frame_capacity = APP_W800_UART_RX_BUF_SIZE;
    ldc_config.frame_mode = LDC_FRAME_MODE_MANUAL;
    ldc_config.full_policy = LDC_FULL_REJECT_NEW;
    ldc_config.lock = app_w800_ldc_lock;
    ldc_config.unlock = app_w800_ldc_unlock;
    if(ldc_init(&g_ldc, &ldc_config) != LDC_STATUS_OK)
    ...
    if(bsp_uart_register_rx_callback(BSP_UART_W800_AT, app_w800_uart_rx, NULL) != 0 ||
       bsp_uart_register_rx_error_callback(BSP_UART_W800_AT,
                                           app_w800_uart_rx_error,
                                           NULL) != 0 ||
...
{
    bsp_irq_state_t state = bsp_irq_lock();
...
void app_w800_task_entry(ULONG thread_input)
{
    ...
    at_w800_access_point_t scan_results[APP_W800_SCAN_MAX_RESULTS];
    ...
    for(;;)
    {
        ...
        app_w800_handle_rx_discontinuity();
        if(at_session_recovery_required(&g_at_session))
        ...
        if(app_w800_take_scan_request())
        {
            uint8_t scan_count = 0U;
            bool scan_ok = at_module_w800_scan_access_points(
                &g_module,
                scan_results,
                APP_W800_SCAN_MAX_RESULTS,
                &scan_count);

            app_w800_publish_scan(scan_ok, scan_results, scan_count);
            continue;
        }
...
void app_w800_get_status(app_w800_status_t *status)
{
    ...
    memset(&status->ldc, 0, sizeof(status->ldc));
    if(ldc_get_stats(&g_ldc, &ldc_stats) == LDC_STATUS_OK)
    {
        status->ldc.rx_bytes = ldc_stats.accepted_bytes;
        status->ldc.packets = ldc_stats.committed_frames;
        status->ldc.overflow = ldc_stats.overflow_frames;
        status->ldc.drop = ldc_stats.dropped_frames;
        status->ldc.overwrite_count = ldc_stats.dropped_frames;
        status->ldc.max_used = ldc_stats.used_high_watermark;
        status->ldc.cur_used = ldc_stats.ready_frames +
                               ldc_stats.claimed_frames;
        status->ldc.packet_used = ldc_stats.ready_frames;
        status->ldc.packet_peak = ldc_stats.ready_high_watermark;
    }
...
app_w800_credentials_result_t app_w800_request_usb_credentials(const char *ssid,
                                                               const char *password)
{
    if(!app_w800_ssid_is_valid(ssid))
...
/** @brief Share the transient credential mailbox with the on-device UI. */
app_w800_credentials_result_t app_w800_request_credentials(const char *ssid,
                                                           const char *password)
{
    return app_w800_request_usb_credentials(ssid, password);
}
...
bool app_w800_request_scan(void)
{
    if(g_scan_mutex_ready == 0U ||
       tx_mutex_get(&g_scan_mutex, TX_WAIT_FOREVER) != TX_SUCCESS)
        return false;
    if(g_scan_requested != 0U ||
       g_scan_snapshot.state == APP_W800_SCAN_PENDING ||
       g_scan_snapshot.state == APP_W800_SCAN_RUNNING)
    {
        (void)tx_mutex_put(&g_scan_mutex);
        return false;
