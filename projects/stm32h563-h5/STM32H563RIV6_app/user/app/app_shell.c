...
static void app_shell_print_ldc(shell_t *shell,
                                const char *name,
                                const app_serial_stats_t *stats)
{
    if(!shell || !name || !stats)
        return;
...
static int app_shell_cmd_touch(shell_t *shell, int argc, char **argv, void *arg)
{
    ...
    (void)shell_printf(shell,
                       "touch %s present=%u int=%u touched=%u points=%u x=%u y=%u raw=%u,%u last_raw=%s%u,%u event=%u gesture=0x%02X chip=0x%02X vendor=0x%02X\r\n",
                       ret == 0 ? "ok" : "error",
                       state.present,
                       state.int_active,
                       state.touched,
