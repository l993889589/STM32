...
{
    if(!bsp_flash_read(address, (uint8_t *)manifest, sizeof(*manifest)))
    {
        return 0U;
    }
...
static uint8_t app_ota_manifest_write(uint32_t address, const ota_manifest_t *manifest)
{
    ...
    return bsp_flash_erase_4k(address) &&
           bsp_flash_write(address, (const uint8_t *)&temp, sizeof(temp)) &&
           bsp_flash_read_verify(address, (const uint8_t *)&temp, sizeof(temp));
}
