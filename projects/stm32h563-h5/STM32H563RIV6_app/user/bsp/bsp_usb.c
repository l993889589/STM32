...
static bsp_status_t bsp_usb_configure_pma(uint32_t endpoint,
                                            uint32_t address)
{
...
/** @brief Implement bsp_usb_init() without CubeMX-generated usb.c. */
bsp_status_t bsp_usb_init(void)
{
    ...
    if((bsp_usb_configure_pma(0x00U, 0x040U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x80U, 0x080U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x01U, 0x0C0U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x81U, 0x100U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x82U, 0x140U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x03U, 0x180U) != BSP_STATUS_OK) ||
       (bsp_usb_configure_pma(0x83U, 0x1C0U) != BSP_STATUS_OK))
    {
        return BSP_STATUS_IO_ERROR;
    }
...
/** @brief Implement bsp_usb_start() after USBX DCD initialization. */
