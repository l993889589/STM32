...
/** @brief Provide the short awake-state settle delay needed by board devices. */
void bsp_power_delay_ms(uint32_t delay_ms)
{
    HAL_Delay(delay_ms);
}

/** @brief Stop HAL tick interrupts while the MCU is entering Stop mode. */
void bsp_power_suspend_tick(void)
{
    HAL_SuspendTick();
}

/** @brief Restart HAL tick interrupts after the caller restores system clocks. */
