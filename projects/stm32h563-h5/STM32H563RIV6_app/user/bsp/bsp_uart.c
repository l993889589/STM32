...
typedef struct
{
    ...
    void *rx_argument;
    bsp_uart_rx_error_cb_t rx_error_callback;
    void *rx_error_argument;
...
/** @brief Register the discontinuity callback for one UART context. */
int bsp_uart_hw_register_rx_error_callback(
    bsp_uart_hw_context_t *context,
    bsp_uart_rx_error_cb_t callback,
    void *argument);
...
/** @brief Implement RX discontinuity callback registration through the board role. */
int bsp_uart_register_rx_error_callback(bsp_uart_port_t port,
                                        bsp_uart_rx_error_cb_t callback,
                                        void *argument)
{
    return bsp_uart_hw_register_rx_error_callback(bsp_uart_get_context(port),
                                                  callback,
                                                  argument);
}
...
/** @brief Bind the protocol owner's UART/DMA discontinuity callback. */
int bsp_uart_hw_register_rx_error_callback(
    bsp_uart_hw_context_t *context,
    bsp_uart_rx_error_cb_t callback,
    void *argument)
{
    ...
    context->rx_error_callback = callback;
    context->rx_error_argument = argument;
    return 0;
}
...
/** @brief Notify the protocol owner that the received byte stream has a gap. */
static void bsp_uart_hw_notify_rx_error(bsp_uart_hw_context_t *context)
{
    if(context != NULL && context->rx_error_callback != NULL)
    {
        context->rx_error_callback(context->port,
                                   context->rx_error_argument);
    }
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *handle, uint16_t size)
{
    ...
    if(discontinuity != 0U)
        bsp_uart_hw_notify_rx_error(context);
}
