...
static RTC_HandleTypeDef bsp_rtc_handle;
static bool bsp_rtc_initialized;
static volatile bool bsp_rtc_wakeup_event;
...
static uint8_t bsp_rtc_days_in_month(uint16_t year, uint8_t month)
{
    ...
    return (month == 2U) && bsp_rtc_is_leap_year(year) ?
           29U : days[month - 1U];
}
...
bsp_status_t bsp_rtc_init(void)
{
    ...
    if(bsp_rtc_initialized)
    {
        return BSP_STATUS_ALREADY_INITIALIZED;
    }
    status = bsp_rtc_configure_clock();
    if(status != BSP_STATUS_OK)
    {
        return status;
    }

    bsp_rtc_handle.Instance = RTC;
    bsp_rtc_handle.Init.HourFormat = RTC_HOURFORMAT_24;
    bsp_rtc_handle.Init.AsynchPrediv = 127U;
    bsp_rtc_handle.Init.SynchPrediv = 255U;
    bsp_rtc_handle.Init.OutPut = RTC_OUTPUT_DISABLE;
    bsp_rtc_handle.Init.OutPutRemap = RTC_OUTPUT_REMAP_NONE;
    bsp_rtc_handle.Init.OutPutPolarity = RTC_OUTPUT_POLARITY_HIGH;
    bsp_rtc_handle.Init.OutPutType = RTC_OUTPUT_TYPE_PUSHPULL;
    bsp_rtc_handle.Init.OutPutPullUp = RTC_OUTPUT_PULLUP_NONE;
    bsp_rtc_handle.Init.BinMode = RTC_BINARY_NONE;
    bsp_rtc_handle.Init.BinMixBcdU = RTC_BINARY_MIX_BCDU_0;
    if(HAL_RTC_Init(&bsp_rtc_handle) != HAL_OK)
    {
        return BSP_STATUS_IO_ERROR;
    }
    bsp_rtc_initialized = true;
    return BSP_STATUS_OK;
}
...
bsp_status_t bsp_rtc_set_datetime(const bsp_rtc_datetime_t *datetime)
{
    ...
    if((HAL_RTC_SetTime(&bsp_rtc_handle, &time, RTC_FORMAT_BIN) != HAL_OK) ||
       (HAL_RTC_SetDate(&bsp_rtc_handle, &date, RTC_FORMAT_BIN) != HAL_OK))
    {
        return BSP_STATUS_IO_ERROR;
    }
    HAL_RTCEx_BKUPWrite(&bsp_rtc_handle,
                        RTC_BKP_DR0,
...
bsp_status_t bsp_rtc_get_datetime(bsp_rtc_datetime_t *datetime)
{
    ...
    if(HAL_RTCEx_BKUPRead(&bsp_rtc_handle, RTC_BKP_DR0) !=
       BOARD_RTC_VALID_MARKER)
    {
        return BSP_STATUS_NOT_READY;
    }
    if((HAL_RTC_GetTime(&bsp_rtc_handle, &time, RTC_FORMAT_BIN) != HAL_OK) ||
       (HAL_RTC_GetDate(&bsp_rtc_handle, &date, RTC_FORMAT_BIN) != HAL_OK))
    {
        return BSP_STATUS_IO_ERROR;
    }
...
bsp_status_t bsp_rtc_is_datetime_valid(bool *is_valid)
{
    ...
    *is_valid = HAL_RTCEx_BKUPRead(&bsp_rtc_handle, RTC_BKP_DR0) ==
                BOARD_RTC_VALID_MARKER;
    return BSP_STATUS_OK;
}

/** @brief Arm the retained one-hertz RTC wakeup counter. */
bsp_status_t bsp_rtc_schedule_wakeup(uint32_t seconds)
{
    ...
    (void)HAL_RTCEx_DeactivateWakeUpTimer(&bsp_rtc_handle);
    bsp_rtc_wakeup_event = false;
    __HAL_RTC_WAKEUPTIMER_CLEAR_FLAG(&bsp_rtc_handle, RTC_FLAG_WUTF);
    HAL_NVIC_ClearPendingIRQ(RTC_IRQn);
    HAL_NVIC_SetPriority(RTC_IRQn, 7U, 0U);
    HAL_NVIC_EnableIRQ(RTC_IRQn);
    if(HAL_RTCEx_SetWakeUpTimer_IT(&bsp_rtc_handle,
                                   seconds - 1U,
...
/** @brief Disable the RTC wakeup counter after use or cancellation. */
bsp_status_t bsp_rtc_cancel_wakeup(void)
{
    ...
    if(HAL_RTCEx_DeactivateWakeUpTimer(&bsp_rtc_handle) != HAL_OK)
    {
        return BSP_STATUS_IO_ERROR;
    }
...
/** @brief Atomically consume the interrupt-owned RTC wake-event latch. */
...
/** @brief Route the STM32H5 shared RTC interrupt into the HAL owner. */
void RTC_IRQHandler(void)
{
    HAL_RTCEx_WakeUpTimerIRQHandler(&bsp_rtc_handle);
}

/** @brief Latch one RTC wakeup event without calling RTOS services in ISR context. */
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *handle)
{
    if(handle == &bsp_rtc_handle)
    {
        bsp_rtc_wakeup_event = true;
    }
}
