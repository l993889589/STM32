...
/** @brief Receive-discontinuity callback executed from HAL interrupt context. */
typedef void (*bsp_uart_rx_error_cb_t)(bsp_uart_port_t port, void *argument);
...
/**
 * @brief Register the owner callback for UART/DMA receive discontinuities.
 * @param port Logical UART role.
 * @param callback ISR callback; NULL disables fault delivery.
 * @param argument Caller-owned callback argument with static service lifetime.
 * @return Zero on success, otherwise -1.
 */
