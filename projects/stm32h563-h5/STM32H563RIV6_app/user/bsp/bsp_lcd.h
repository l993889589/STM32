...
/** @brief LCD asynchronous transfer health counters. */
typedef struct
{
    uint32_t dma_flushes_started;
    uint32_t dma_flushes_completed;
    uint32_t dma_bytes;
    uint32_t dma_busy_rejects;
...
int bsp_lcd_draw_rgb565_dma(uint16_t x,
                            ...
                            uint16_t height,
                            const uint16_t *pixels);
/** @brief Copy a consistent snapshot of LCD DMA health counters. */
