 ...
 *   fail-closed recovery after an incomplete payload and explicit peer reset.
 *
 * Usage:
...
int main(void)
{
    ...
    if(!at_session_recovery_required(&session) ||
       !at_session_recover_after_transport_reset(&session))
    {
        (void)fprintf(stderr, "FAIL malformed header did not require explicit reset\n");
        failures++;
    }
    ...
    if(!at_session_recovery_required(&session) ||
       !at_session_recover_after_transport_reset(&session))
    {
        (void)fprintf(stderr, "FAIL oversized payload did not require explicit reset\n");
        failures++;
    }
    ...
    if(!at_session_recovery_required(&session) ||
       !at_session_recover_after_transport_reset(&session))
    {
        (void)fprintf(stderr, "FAIL partial payload did not require explicit reset\n");
        failures++;
    }

    test_prepare(&transport, NULL, 0U);
    if(at_session_cmd_read_binary_ex(&session, "AT+NOHEADER", "+OK=", output,
                                     sizeof(output), &output_len,
                                     AT_RAW_SEPARATOR_EMPTY_LINE, 20U, 1U) ||
       !at_session_recovery_required(&session))
    {
        (void)fprintf(stderr, "FAIL missing header did not latch recovery\n");
        failures++;
    }

    memset(output, 0, sizeof(output));
    test_prepare(&transport, recovery_response, sizeof(recovery_response) - 1U);
    if(at_session_cmd_read_binary_ex(&session, "AT+UNSAFE", "+OK=", output,
                                     sizeof(output), &output_len,
                                     AT_RAW_SEPARATOR_EMPTY_LINE, 50U, 1U) ||
       transport.tx_count != 0U)
    {
        (void)fprintf(stderr, "FAIL unsafe binary command was transmitted before reset\n");
        failures++;
    }
    if(!at_session_recover_after_transport_reset(&session))
    {
        (void)fprintf(stderr, "FAIL explicit transport reset recovery\n");
        failures++;
    }
    test_prepare(&transport, recovery_response, sizeof(recovery_response) - 1U);
    ...
    if(diag.attempts != 7U || diag.successes != 3U ||
    ...
    (void)printf("PASS at_core binary framing: 7 cases\n");
    return 0;
}
