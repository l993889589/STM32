...
| --- | --- | --- | --- | --- |
| 状态灯 | PC12 | 低有效；启动默认熄灭 | `bsp_gpio.c` | `compile_complete` |
| W800 BOOT | PA8 | 启动默认低 | `bsp_gpio.c` | `compile_complete` |
| W800 RESET | PC9 | 低有效；启动先保持复位 | `bsp_gpio.c` | `compile_complete` |
| W800 WAKE | PC8 | 启动默认低 | `bsp_gpio.c` | `compile_complete` |
| LCD CS/DC/RESET | PD11/PD12/PB4 | CS默认释放、RESET低有效 | `bsp_gpio.c` | `compile_complete` |
| Flash CS | PA4 | 低有效，默认释放 | `bsp_gpio.c` | `compile_complete` |
| Touch INT/RESET | PB14/PB15 | INT输入、RESET低有效 | `bsp_gpio.c` | `compile_complete` |
| USB ID | PB13 | 只读输入、无内部上下拉 | `bsp_gpio.c` | `compile_complete` |
...
| USB FS Device | PA11 DM / PA12 DP，HSI48，静态 PMA 布局 | `bsp_usb.c` 独占 PCD 和 IRQ，USBX 只经适配层访问 | `compile_complete` |
