...
static mcu_uart_context_t board_uart_contexts[BOARD_UART_COUNT];
...
bsp_status_t bsp_uart_init(board_uart_role_t role, const bsp_uart_config_t *config)
{
    ...
    return status == BSP_STATUS_OK ?
           mcu_uart_init(&board_uart_contexts[role], instance, config) : status;
}
                               ...
                               uint32_t *length)
{
    return role >= BOARD_UART_COUNT ? BSP_STATUS_INVALID_ARGUMENT :
           mcu_uart_try_read(&board_uart_contexts[role], data, capacity, length);
}

/**
 * @brief Implement bsp_uart_try_read_timed() as documented by its interface contract.
 */
...
{
    return role >= BOARD_UART_COUNT ? BSP_STATUS_INVALID_ARGUMENT :
           mcu_uart_try_read_timed(&board_uart_contexts[role],
                                   data,
                                   timestamp_us,
                                   capacity,
                                   length);
...
{
    return role >= BOARD_UART_COUNT ? BSP_STATUS_INVALID_ARGUMENT :
           mcu_uart_write(&board_uart_contexts[role], data, length, timeout_ms);
}

/**
 * @brief Implement bsp_uart_get_health() as documented by its interface contract.
 */
bsp_status_t bsp_uart_get_health(board_uart_role_t role,
                                      bsp_uart_health_t *health)
{
    if((role >= BOARD_UART_COUNT) || (health == NULL))
    {
        return BSP_STATUS_INVALID_ARGUMENT;
    }
    *health = board_uart_contexts[role].health;
    return board_uart_contexts[role].is_initialized ? BSP_STATUS_OK : BSP_STATUS_NOT_READY;
}

/**
 * @brief Implement bsp_uart_get_baud_rate() as documented by its interface contract.
 */
bsp_status_t bsp_uart_get_baud_rate(board_uart_role_t role,
                                    uint32_t *baud_rate)
{
    return role >= BOARD_UART_COUNT ? BSP_STATUS_INVALID_ARGUMENT :
           mcu_uart_get_baud_rate(&board_uart_contexts[role], baud_rate);
}

/** @brief Dispatch the USART1 vector to the Wi-Fi UART owner. */
void USART1_IRQHandler(void)
{
    mcu_uart_irq(&board_uart_contexts[BOARD_UART_WIFI]);
}

/** @brief Dispatch the USART2 vector to the first RS-485 UART owner. */
void USART2_IRQHandler(void)
{
    mcu_uart_irq(&board_uart_contexts[BOARD_UART_RS485_1]);
}

/** @brief Dispatch the USART3 vector to the debug UART owner. */
void USART3_IRQHandler(void)
{
    mcu_uart_irq(&board_uart_contexts[BOARD_UART_DEBUG]);
}

/** @brief Dispatch the UART4 vector to the second RS-485 UART owner. */
void UART4_IRQHandler(void)
{
    mcu_uart_irq(&board_uart_contexts[BOARD_UART_RS485_2]);
}
