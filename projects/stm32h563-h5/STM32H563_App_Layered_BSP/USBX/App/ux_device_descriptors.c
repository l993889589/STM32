...
#endif
    case CLASS_TYPE_VENDOR:
    {
      static USBD_IfDescTypedef *pIfDesc;
      static USBD_EpDescTypedef *pEpDesc;
      uint32_t pConf = (uint32_t)pCmpstConfDesc;
      uint32_t *Sze = &pdev->CurrConfDescSz;
      ...
      if(Speed == USBD_HIGH_SPEED)
      {
        USBD_FrameWork_AssignEp(pdev, USBD_VENDOR_EPOUT_ADDR,
                                USBD_EP_TYPE_BULK, USBD_VENDOR_EPOUT_HS_MPS);
        USBD_FrameWork_AssignEp(pdev, USBD_VENDOR_EPIN_ADDR,
                                USBD_EP_TYPE_BULK, USBD_VENDOR_EPIN_HS_MPS);
      }
      else
      {
        USBD_FrameWork_AssignEp(pdev, USBD_VENDOR_EPOUT_ADDR,
                                USBD_EP_TYPE_BULK, USBD_VENDOR_EPOUT_FS_MPS);
        USBD_FrameWork_AssignEp(pdev, USBD_VENDOR_EPIN_ADDR,
                                USBD_EP_TYPE_BULK, USBD_VENDOR_EPIN_FS_MPS);
      }

      __USBD_FRAMEWORK_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, 2U,
                              0x99U, 0x99U, 0x99U, 0U);
      __USBD_FRAMEWORK_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add,
                              USBD_EP_TYPE_BULK,
                              pdev->tclasslist[pdev->classId].Eps[0].size,
                              0U, 0U);
      __USBD_FRAMEWORK_SET_EP(pdev->tclasslist[pdev->classId].Eps[1].add,
                              USBD_EP_TYPE_BULK,
                              pdev->tclasslist[pdev->classId].Eps[1].size,
                              0U, 0U);
      ((USBD_ConfigDescTypedef *)pCmpstConfDesc)->bNumInterfaces += 1U;
      ((USBD_ConfigDescTypedef *)pCmpstConfDesc)->wDescriptorLength = pdev->CurrConfDescSz;
      break;
    }
#if defined(__clang__)
