/*
 * LVGL UI asset store backed by GD25LQ128 external SPI NOR.
 *
 * Purpose:
...
static uint8_t read_exact(uint32_t address, uint8_t *data, uint32_t len)
{
    while(len != 0U)
    {
        ...
        if(!gd25lq128_read(address, data, chunk))
            return 0U;
        address += chunk;
        data += chunk;
        len -= chunk;
...
static uint8_t erase_range(uint32_t base, uint32_t len)
{
    ...
    {
        if(!gd25lq128_erase_4k(base + offset))
            return 0U;
    }
    return 1U;
}
...
static uint8_t flush_update_sector(void)
{
    ...
    {
        if(gd25lq128_erase_4k(address) &&
           gd25lq128_write(address, s_update_sector_buf, sizeof(s_update_sector_buf)) &&
           gd25lq128_read_verify(address, s_update_sector_buf, sizeof(s_update_sector_buf)))
        {
...
static uint8_t verify_slot_image_data(const ui_asset_slot_t *slot)
{
    ...
    for(uint32_t i = 0U; i < UI_ASSET_PAGE_COUNT; i++)
    {
        ...
        while(remain != 0U)
        {
            ...
            if(!gd25lq128_read(address, s_update_header_sector, chunk))
                return 0U;
...
bool ui_asset_update_commit(void)
{
    ...
    if(!gd25lq128_erase_4k(s_update.base) ||
       !gd25lq128_write(s_update.base, s_update_header_sector, sizeof(s_update_header_sector)) ||
       !gd25lq128_read_verify(s_update.base, s_update_header_sector, sizeof(s_update_header_sector)))
    {
    ...
    {
        (void)gd25lq128_erase_4k(s_update.base);
