...
static const bsp_clock_config_t g_board_clock_config =
{
...
int bsp_init(void)
{
    ...
    status = bsp_clock_init(&g_board_clock_config);
    ...
    if((bsp_board_init() != 0) || (bsp_uart_init() != 0))
    {
        return -1;
    }
...
void bsp_led_on(bsp_led_t led)
{
    ...
    {
        (void)board_gpio_write(BOARD_GPIO_STATUS_LED, true);
    }
}
...
void bsp_led_off(bsp_led_t led)
{
    if(led == BSP_LED_STATUS)
    {
        (void)board_gpio_write(BOARD_GPIO_STATUS_LED, false);
    }
}
...
void bsp_led_toggle(bsp_led_t led)
{
    if(led == BSP_LED_STATUS)
    {
        (void)board_gpio_toggle(BOARD_GPIO_STATUS_LED);
    }
}
...
{
    (void)board_gpio_write(BOARD_GPIO_W800_RESET, true);
}

/** @brief Release the W800 hardware reset output. */
void bsp_w800_reset_release(void)
{
    (void)board_gpio_write(BOARD_GPIO_W800_RESET, false);
}
