...
static void lcd_spi_tx_complete(board_spi_role_t role,
                                bsp_status_t status,
                                void *argument);
...
{
    (void)board_gpio_write(BOARD_GPIO_LCD_DC, false);
}

/** @brief Select data mode on the display D/C signal. */
static void lcd_data_mode(void)
{
    (void)board_gpio_write(BOARD_GPIO_LCD_DC, true);
}
...
static int lcd_start_next_dma_chunk(void)
{
    ...
    (void)bsp_cache_clean(s_lcd_tx_chunk, bytes_now);
...
{
    (void)board_gpio_write(BOARD_GPIO_LCD_RESET, true);
    HAL_Delay(20U);
    (void)board_gpio_write(BOARD_GPIO_LCD_RESET, false);
    HAL_Delay(120U);
}
