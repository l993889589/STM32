...
static volatile bool board_rtc_wakeup_event;
...
bsp_status_t bsp_rtc_schedule_wakeup(uint32_t seconds)
{
    ...
    (void)HAL_RTCEx_DeactivateWakeUpTimer(&board_rtc_handle);
    board_rtc_wakeup_event = false;
    __HAL_RTC_WAKEUPTIMER_CLEAR_FLAG(&board_rtc_handle, RTC_FLAG_WUTF);
    HAL_NVIC_ClearPendingIRQ(RTC_IRQn);
    HAL_NVIC_SetPriority(RTC_IRQn, 7U, 0U);
    HAL_NVIC_EnableIRQ(RTC_IRQn);
    if(HAL_RTCEx_SetWakeUpTimer_IT(&board_rtc_handle,
                                   seconds - 1U,
...
bsp_status_t bsp_rtc_cancel_wakeup(void)
{
    ...
    if(HAL_RTCEx_DeactivateWakeUpTimer(&board_rtc_handle) != HAL_OK)
    {
        return BSP_STATUS_IO_ERROR;
    }
...
{
    HAL_RTCEx_WakeUpTimerIRQHandler(&board_rtc_handle);
}
...
{
    if(handle == &board_rtc_handle)
    {
        board_rtc_wakeup_event = true;
    }
}
