/**
 * @file mcu_pwm.c
 * @brief STM32H5 timer/PWM physical-unit solver and runtime implementation.
 */
...
static bsp_status_t mcu_pwm_solve(uint32_t timer_clock_hz,
                                  uint8_t counter_width_bits,
                                  uint32_t requested_frequency_hz,
                                  mcu_pwm_solution_t *solution)
{
...
static bsp_status_t mcu_pwm_apply(mcu_pwm_context_t *context,
                                  ...
                                  bsp_pwm_result_t *result)
{
    ...
    status = mcu_pwm_solve(timer_clock_hz,
...
/** @brief Implement mcu_pwm_init() for one statically owned context. */
                          ...
                          bsp_pwm_result_t *result)
{
    ...
    status = mcu_pwm_apply(context, config, result);
    if(status == BSP_STATUS_OK)
    {
...
/** @brief Implement mcu_pwm_configure() for an initialized context. */
bsp_status_t mcu_pwm_configure(mcu_pwm_context_t *context,
                               const bsp_pwm_config_t *config,
                               bsp_pwm_result_t *result)
{
    return (context == NULL) || !context->is_initialized ?
           BSP_STATUS_NOT_READY : mcu_pwm_apply(context, config, result);
}

/** @brief Implement mcu_pwm_start() for one timer channel. */
...
/** @brief Implement mcu_pwm_stop() for one timer channel. */
