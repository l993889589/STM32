...
static volatile bool board_touch_wakeup_event;
static uint32_t board_saved_nvic_enable[5];
static uint32_t board_saved_systick_control;
static bool board_nvic_mask_active;

/** @brief Forward ordered USB shutdown through its private board owner. */
bsp_status_t bsp_power_prepare_usb(void)
{
    return board_usb_prepare_stop();
}

/** @brief Forward ordered USB restart after the HSI48 clock is stable. */
bsp_status_t bsp_power_resume_usb(void)
{
    return board_usb_resume_after_stop();
}
...
bsp_status_t bsp_power_configure_touch_wakeup(bool enable)
{
    ...
    board_touch_wakeup_event = false;
...
bool bsp_power_take_touch_wakeup_event(void)
{
    ...
    __disable_irq();
    event = board_touch_wakeup_event;
    board_touch_wakeup_event = false;
    if(primask == 0U)
    {
        __enable_irq();
                                                  ...
                                                  bool w800_wakeup)
{
    ...
    if(board_nvic_mask_active)
    {
        return BSP_STATUS_CONFLICT;
    }
    for(index = 0U; index < 5U; index++)
    {
        board_saved_nvic_enable[index] = NVIC->ISER[index];
        NVIC->ICER[index] = 0xFFFFFFFFUL;
    }
    board_saved_systick_control = SysTick->CTRL;
    SysTick->CTRL = board_saved_systick_control & ~SysTick_CTRL_TICKINT_Msk;
...
void bsp_power_restore_interrupts(void)
{
    ...
    {
        NVIC->ICER[index] = 0xFFFFFFFFUL;
        NVIC->ISER[index] = board_saved_nvic_enable[index];
    }
    SysTick->CTRL = board_saved_systick_control;
    __DSB();
    __ISB();
    board_nvic_mask_active = false;
}
