...
uint8_t bsp_touch_int_active(void)
{
    ...
    return board_gpio_read(BOARD_GPIO_TOUCH_INTERRUPT, &is_active) == BSP_STATUS_OK &&
           is_active ? 1U : 0U;
}
...
int bsp_touch_init(void)
{
    ...
    (void)board_gpio_write(BOARD_GPIO_TOUCH_RESET, true);
    bsp_dwt_delay_ms(10U);
    (void)board_gpio_write(BOARD_GPIO_TOUCH_RESET, false);
