...
typedef struct
{
    ...
    bool is_output;
    bool safe_active;
} board_gpio_descriptor_t;

static const board_gpio_descriptor_t g_board_gpio_descriptors[BOARD_GPIO_COUNT] =
{
...
static bool g_board_gpio_initialized;

/** @brief Convert a logical state to the configured electrical level. */
static GPIO_PinState board_gpio_to_level(const board_gpio_descriptor_t *descriptor,
                                         bool is_active)
{
...
static bool board_gpio_from_level(const board_gpio_descriptor_t *descriptor,
                                  GPIO_PinState level)
{
...
/** @brief Implement board_gpio_init() and apply every output safe state first. */
bsp_status_t board_gpio_init(void)
{
    GPIO_InitTypeDef gpio = {0};
    uint32_t index;

    if(g_board_gpio_initialized)
    {
    ...
    {
        const board_gpio_descriptor_t *descriptor = &g_board_gpio_descriptors[index];
        ...
        if(descriptor->is_output)
        {
            HAL_GPIO_WritePin(descriptor->port,
                              descriptor->pin,
                              board_gpio_to_level(descriptor,
                                                  descriptor->safe_active));
        }
        HAL_GPIO_Init(descriptor->port, &gpio);
    }

    g_board_gpio_initialized = true;
    return BSP_STATUS_OK;
}

/** @brief Implement board_gpio_write() for board-owned outputs. */
bsp_status_t board_gpio_write(board_gpio_role_t role, bool is_active)
{
    const board_gpio_descriptor_t *descriptor;
    ...
    descriptor = &g_board_gpio_descriptors[role];
    if(!descriptor->is_output)
    {
    ...
    HAL_GPIO_WritePin(descriptor->port,
                      descriptor->pin,
                      board_gpio_to_level(descriptor, is_active));
    return BSP_STATUS_OK;
}

/** @brief Implement board_gpio_read() for input and output roles. */
bsp_status_t board_gpio_read(board_gpio_role_t role, bool *is_active)
{
    const board_gpio_descriptor_t *descriptor;
    ...
    descriptor = &g_board_gpio_descriptors[role];
    *is_active = board_gpio_from_level(descriptor,
                                       HAL_GPIO_ReadPin(descriptor->port,
                                                        descriptor->pin));
    return BSP_STATUS_OK;
}

/** @brief Implement board_gpio_toggle() while retaining logical polarity. */
bsp_status_t board_gpio_toggle(board_gpio_role_t role)
{
    bool is_active;
    bsp_status_t status = board_gpio_read(role, &is_active);

    if(status != BSP_STATUS_OK)
    {
        return status;
    }
    if(!g_board_gpio_descriptors[role].is_output)
    {
        return BSP_STATUS_NOT_SUPPORTED;
    }
    return board_gpio_write(role, !is_active);
}
