...
bsp_status_t board_init(void)
{
    ...
    reset_flags = RCC->RSR;
    bsp_diagnostics_init(reset_flags);
    __HAL_RCC_CLEAR_RESET_FLAGS();

    status = board_early_safe_gpio_init();
    if(status != BSP_STATUS_OK)
    {
        bsp_diagnostics_record_fatal(BSP_FATAL_STAGE_SAFE_GPIO, status);
        return status;
    }
    ...
    if(status != BSP_STATUS_OK)
    {
        bsp_diagnostics_record_fatal(BSP_FATAL_STAGE_CLOCK, status);
        return status;
    }
    ...
    if((status != BSP_STATUS_OK) && (status != BSP_STATUS_ALREADY_INITIALIZED))
    {
        bsp_diagnostics_record_fatal(BSP_FATAL_STAGE_BOARD, status);
        return status;
    }
