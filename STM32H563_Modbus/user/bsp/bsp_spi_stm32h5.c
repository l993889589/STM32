 ...
 */
static bool bsp_spi_solve_prescaler(uint32_t kernel_clock_hz,
...
static bsp_status_t bsp_spi_from_hal(HAL_StatusTypeDef status)
{
    if(status == HAL_OK)
    {
...
/**
 * @brief Implement bsp_spi_stm32h5_init() as documented by its interface contract.
 */
                                  ...
                                  const bsp_spi_config_t *config)
{
    ...
    if(!bsp_spi_solve_prescaler(kernel_clock_hz,
                                config->baud_rate_hz,
                                &prescaler,
...
/**
 * @brief Implement bsp_spi_stm32h5_write() as documented by its interface contract.
 */
                                   ...
                                   uint32_t timeout_ms)
{
    ...
    return bsp_spi_from_hal(HAL_SPI_Transmit(&context->handle,
                                             (uint8_t *)data,
                                             (uint16_t)length,
                                             timeout_ms));
}
...
/**
 * @brief Implement bsp_spi_stm32h5_read() as documented by its interface contract.
 */
                                  ...
                                  uint32_t timeout_ms)
{
    ...
    return bsp_spi_from_hal(HAL_SPI_Receive(&context->handle,
                                            data,
                                            (uint16_t)length,
                                            timeout_ms));
}
