...
static mcu_fdcan_context_t *fdcan_contexts[2];

/** @brief Return an absolute difference without signed arithmetic. */
static uint32_t mcu_fdcan_abs_difference(uint32_t left, uint32_t right)
{
    return left >= right ? left - right : right - left;
}

/** @brief Locate the static context registered for a HAL handle. */
static mcu_fdcan_context_t *mcu_fdcan_find_context(
...
static bsp_status_t mcu_fdcan_register_context(
    mcu_fdcan_context_t *context)
{
    uint32_t index;
...
static bsp_status_t mcu_fdcan_solve_timing(uint32_t kernel_clock_hz,
    ...
    for(prescaler = 1U; prescaler <= maximum_prescaler; ++prescaler)
    {
            ...
            ++total_quanta)
        {
            ...
            for(delta = -1; delta <= 1; ++delta)
            {
                ...
                bitrate_error = mcu_fdcan_abs_difference(bitrate,
                ...
                sample_point = (uint16_t)(((1U + segment_1) * 1000U) /
                                          total_quanta);
                sample_error = mcu_fdcan_abs_difference(sample_point,
...
static bsp_status_t mcu_fdcan_length_to_dlc(uint8_t length, uint32_t *dlc)
{
...
static uint8_t mcu_fdcan_dlc_to_length(uint32_t dlc)
{
    static const uint8_t lengths[16] =
    {
        0U, 1U, 2U, 3U, 4U, 5U, 6U, 7U,
...
static bsp_status_t mcu_fdcan_configure_filters(FDCAN_HandleTypeDef *handle)
{
                                    ...
                                    const bsp_fdcan_config_t *config)
{
    ...
    status = mcu_fdcan_solve_timing(kernel_clock_hz,
    ...
    status = mcu_fdcan_configure_filters(&context->handle);
    if(status != BSP_STATUS_OK)
    {
        return status;
    }
    status = mcu_fdcan_register_context(context);
    if(status != BSP_STATUS_OK)
    {
        return status;
    }
                                    ...
                                    const bsp_fdcan_frame_t *frame)
{
    ...
    status = mcu_fdcan_length_to_dlc(frame->length, &dlc);
    if(status != BSP_STATUS_OK)
    {
        return status;
    }
    ...
    if(HAL_FDCAN_AddMessageToTxFifoQ(&context->handle,
                                     ...
                                     frame->data) != HAL_OK)
    {
        context->health.last_hal_error = HAL_FDCAN_GetError(&context->handle);
        return BSP_STATUS_IO_ERROR;
    }
                                           ...
                                           bool *has_frame)
{
    ...
    *has_frame = false;
    ...
    frame->length = mcu_fdcan_dlc_to_length(header.DataLength);
    *has_frame = true;
...
{
    mcu_fdcan_context_t *context = mcu_fdcan_find_context(handle);
                                   ...
                                   uint32_t interrupt_flags)
{
    ...
    context->health.last_hal_error = HAL_FDCAN_GetError(handle);
}
