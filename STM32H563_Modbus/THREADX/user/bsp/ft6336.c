...
#define FT6336_ADDRESS_7BIT       (0x38U)
#define FT6336_REG_GESTURE        (0x01U)
#define FT6336_REG_VENDOR_ID      (0xA3U)
#define FT6336_REG_CHIP_ID_A      (0xA6U)
#define FT6336_REG_CHIP_ID_B      (0xA8U)
#define FT6336_TRANSFER_TIMEOUT_MS (20U)
#define FT6336_RESET_HOLD_MS      (10U)
#define FT6336_STARTUP_MS         (300U)

static bool ft6336_is_initialized;

/**
 * @brief Read one FT6336U register.
 */
static bsp_status_t ft6336_read_register(uint8_t register_address,
                                             uint8_t *value)
{
    return bsp_i2c_memory_read(BOARD_I2C_TOUCH,
                               ...
                               value,
                               1U,
                               FT6336_TRANSFER_TIMEOUT_MS);
}

/**
 * @brief Decode one six-byte FT6336U touch-point record.
 */
static void ft6336_decode_point(const uint8_t *data,
...
/**
 * @brief Implement ft6336_init() as documented by its interface contract.
 */
bsp_status_t ft6336_init(ft6336_identity_t *identity)
{
    ...
    ft6336_identity_t detected = {0};
    bsp_status_t status;

    if(ft6336_is_initialized)
    {
    ...
    if(status == BSP_STATUS_OK)
    {
        bsp_time_delay_ms(FT6336_RESET_HOLD_MS);
    ...
    bsp_time_delay_ms(FT6336_STARTUP_MS);

    status = ft6336_read_register(FT6336_REG_VENDOR_ID,
                                      &detected.vendor_id);
    if(status == BSP_STATUS_OK)
    {
        status = ft6336_read_register(FT6336_REG_CHIP_ID_A,
                                          &detected.chip_id);
    }
    if((status == BSP_STATUS_OK) &&
       ((detected.chip_id == 0U) || (detected.chip_id == UINT8_MAX)))
    {
        status = ft6336_read_register(FT6336_REG_CHIP_ID_B,
                                          &detected.chip_id);
    }
    if(status != BSP_STATUS_OK)
    {
...
/**
 * @brief Implement ft6336_interrupt_is_active() as documented by its interface contract.
 */
bsp_status_t ft6336_interrupt_is_active(bool *is_active)
{
    return ft6336_is_initialized ?
           board_control_read(BOARD_CONTROL_TOUCH_INTERRUPT, is_active) :
           BSP_STATUS_NOT_READY;
}

/**
 * @brief Implement ft6336_read_state() as documented by its interface contract.
 */
bsp_status_t ft6336_read_state(ft6336_state_t *state)
{
    ...
    {
        ft6336_decode_point(&data[2], &state->points[0]);
    }
    if(state->point_count > 1U)
    {
        ft6336_decode_point(&data[8], &state->points[1]);
    }
    return BSP_STATUS_OK;
}
