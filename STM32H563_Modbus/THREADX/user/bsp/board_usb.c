...
static bsp_usb_handlers_t board_usb_handlers;
static bsp_usb_health_t board_usb_health;
...
bsp_status_t bsp_usb_init(const bsp_usb_handlers_t *handlers)
{
    bsp_status_t status;
    ...
    memset(&board_usb_health, 0, sizeof(board_usb_health));
...
bsp_status_t bsp_usb_start(void)
{
    ...
    if(board_usb_health.started)
    {
        return BSP_STATUS_OK;
    }
    if(HAL_PCD_Start(&board_usb_handle) != HAL_OK)
    {
        ++board_usb_health.errors;
        board_usb_health.last_hal_error = board_usb_handle.ErrorCode;
        return BSP_STATUS_IO_ERROR;
    }
    board_usb_health.started = true;
    return BSP_STATUS_OK;
}

/** @brief Stop and disconnect the initialized USB controller. */
bsp_status_t bsp_usb_stop(void)
{
    if(!board_usb_initialized)
    {
        return BSP_STATUS_NOT_READY;
    ...
    board_usb_health.started = false;
    board_usb_health.connected = false;
    return BSP_STATUS_OK;
}

/** @brief Program a host-assigned USB device address. */
bsp_status_t bsp_usb_set_address(uint8_t address)
{
    if(address > 127U)
    {
        return BSP_STATUS_INVALID_ARGUMENT;
...
bsp_status_t bsp_usb_endpoint_open(uint8_t endpoint_address,
                                          uint16_t maximum_packet_size,
                                          bsp_usb_endpoint_type_t type)
{
    uint8_t hal_type;
...
bsp_status_t bsp_usb_endpoint_close(uint8_t endpoint_address)
{
    if(!board_usb_endpoint_address_is_valid(endpoint_address))
    {
        return BSP_STATUS_INVALID_ARGUMENT;
...
bsp_status_t bsp_usb_transmit(uint8_t endpoint_address,
                                     uint8_t *data,
                                     uint32_t length)
{
    if(!board_usb_endpoint_address_is_valid(endpoint_address) ||
...
bsp_status_t bsp_usb_receive(uint8_t endpoint_address,
                                    uint8_t *data,
                                    uint32_t capacity)
{
    if(!board_usb_endpoint_address_is_valid(endpoint_address) ||
...
bsp_status_t bsp_usb_endpoint_set_stall(uint8_t endpoint_address,
                                               bool is_stalled)
{
...
bsp_status_t bsp_usb_endpoint_flush(uint8_t endpoint_address)
{
    if(!board_usb_endpoint_address_is_valid(endpoint_address))
    {
        return BSP_STATUS_INVALID_ARGUMENT;
...
/** @brief Copy current USB controller health. */
bsp_status_t bsp_usb_get_health(
...
void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *handle)
{
    ...
    ++board_usb_health.setup_packets;
                                  ...
                                  uint8_t endpoint_number)
{
    ...
    ++board_usb_health.out_completions;
                                 ...
                                 uint8_t endpoint_number)
{
    ...
    ++board_usb_health.in_completions;
...
void HAL_PCD_SuspendCallback(PCD_HandleTypeDef *handle)
{
    ...
    ++board_usb_health.suspends;
    board_usb_health.suspended = true;
...
void HAL_PCD_ResumeCallback(PCD_HandleTypeDef *handle)
{
    ...
    ++board_usb_health.resumes;
    board_usb_health.suspended = false;
...
void HAL_PCD_ConnectCallback(PCD_HandleTypeDef *handle)
{
    ...
    ++board_usb_health.connects;
    board_usb_health.connected = true;
...
void HAL_PCD_SOFCallback(PCD_HandleTypeDef *handle)
{
    ...
    ++board_usb_health.start_of_frames;
