...
{
    return role >= BOARD_UART_COUNT ? BSP_STATUS_INVALID_ARGUMENT :
           mcu_uart_get_config(&board_uart_contexts[role], config);
}
...
{
    mcu_uart_dma_irq(&board_uart_contexts[BOARD_UART_RS485_1]);
}

/** @brief Dispatch GPDMA1 channel 2 to the RS-485-1 transmit owner. */
void GPDMA1_Channel2_IRQHandler(void)
{
    mcu_uart_tx_dma_irq(&board_uart_contexts[BOARD_UART_RS485_1]);
}
