...
static mcu_spi_context_t board_spi_contexts[BOARD_SPI_COUNT];
...
bsp_status_t bsp_spi_init(board_spi_role_t role, const bsp_spi_config_t *config)
{
    ...
    return mcu_spi_init(&board_spi_contexts[role],
                                instance,
                                kernel_clock_hz,
                                config);
}
                           ...
                           uint32_t timeout_ms)
{
    return role < BOARD_SPI_COUNT ?
           mcu_spi_write(&board_spi_contexts[role],
                                 data,
                                 length,
                                 timeout_ms) :
...
{
    return role < BOARD_SPI_COUNT ?
           mcu_spi_read(&board_spi_contexts[role],
                                data,
                                length,
                                timeout_ms) :
           BSP_STATUS_INVALID_ARGUMENT;
