...
| `BOARD_WIFI_BOOT` | `w800` | 可选 | PA8 GPIO | 极性和复位采样时序集中在 board config；默认普通启动 | 无 | 实现可配置启动模式 | `provisional` |
| `BOARD_WIFI_RESET` | `w800` | 可选 | PC9 GPIO | 按参考固件配置 active low；早期保持无效高电平，驱动初始化时执行有界复位脉冲 | 无 | 实现复位时序 | `provisional` |
| `BOARD_WIFI_WAKE` | `w800` | 可选 | PC8 GPIO | 按参考固件配置 active high；默认不请求唤醒 | 可选 EXTI 不启用 | 实现电平控制 | `provisional` |
...
| `BOARD_FLASH_CS` | `gd25lq128` | 必需 | PA4 GPIO | active low；最早期先输出高 | 无 | JEDEC ID | `verified_binding` |
...
| `BOARD_TOUCH_INTERRUPT` | `ft6336` | 可选 | PB14 GPIO input | 参考器件 active low；第一版轮询读取 | 不启用 EXTI | 实现只读状态 | `provisional` |
| `BOARD_TOUCH_RESET` | `ft6336` | 可选 | PB15 GPIO | 参考器件 active low；最早期保持复位 | 无 | 实现有界复位时序 | `provisional` |
...
| --- | --- | --- | --- |
| 系统 RCC/PLL/总线时钟 | `Core/Src/main.c` 的 `system_clock_config` | `mcu_clock` 只读查询 | 驱动自行改 PLL 或 APB 分频 |
| 裸机 SysTick | `Core/Src/stm32h5xx_it.c` | HAL deadline、诊断 | 外设驱动修改 SysTick 配置 |
| TIM2 base + CH4 | `mcu_pwm` | `display_backlight` | LCD 驱动直接修改 PSC/ARR |
| USART1/2/3/UART4 | `mcu_uart` | 各逻辑 transport owner | 应用拿 HAL handle 或重初始化 UART |
| UART RX 静态缓冲区 | 各 UART transport context | 对应 LDC/service | 多服务共享同一 mutable ring |
| SPI1 总线 | `spi_flash_bus` | `gd25lq128` | Flash driver 重新配置共享 bus |
| PA4 Flash CS | `gd25lq128` | 无其他 owner | 通用 GPIO API 任意操作 |
| SPI2 总线 | `spi_display_bus` | `st7796` | LCD driver 内部调用 `MX_SPI2_Init()` 或重配 SPI2 |
| I2C1 总线 | `i2c_touch_bus` | `ft6336` | Touch driver 重新初始化共享 bus |
| FDCAN1/FDCAN2 | 各 `mcu_fdcan` context | 对应 field service | 应用直接修改 filter/message RAM |
| USB DRD FS | `bsp_usb` | 上层 device/class stack adapter | 应用获取 PCD handle 或重复定义 HAL callbacks |
| RTC + backup domain | `board_rtc` | 时间服务 | 其他模块修改 LSE/RTC prescaler 或擦除 backup domain |
| IRQ vectors/HAL callbacks | 对应 `board_uart`、`board_fdcan`、`board_usb` 唯一 owner | 静态 owner dispatch | 各模块重复定义同一 vector/HAL callback |
...
5. 复用 `bsp_*` 公共 API、`mcu_*` 机制、LDC、OSAL API 和上层服务；
