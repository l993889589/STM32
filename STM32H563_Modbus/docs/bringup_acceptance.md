...
- 平铺的 `user/bsp`，以 `bsp_*`、`board_*` 和 `*_stm32h5` 区分逻辑边界；
...
- fatal stage/status 和 fault snapshot；
- clock query service。
...
- 通过有界 superloop 调用 shared `poll/step`；
- 不依赖 RTOS 头文件或对象；
- 在等待期间按契约推进必要服务；
...
- ISR 最大工作量有界；
- bare-metal/ThreadX 使用同一 shared source；
- 诊断 counters 可读取。
