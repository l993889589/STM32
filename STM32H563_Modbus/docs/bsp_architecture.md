...
>
> 当前实施例外：本轮原理图全外设实现只进入 `luoji`，`THREADX` 冻结；新增公共 API 不含裸机专有类型，以便后续按文件复制到 RTOS 工程，而不是形成编译期共享目录。
...
- `user/bsp/bsp_*`：状态码、时钟查询、deadline、临界区和公共外设 API。
- `user/bsp/*_stm32h5`：STM32H5 私有机制，不作为应用接口。
- `user/bsp/board_*`：引脚、AF、极性、安全电平、实例、IRQ/DMA 分配和初始化组合。
- `user/bsp/drv_*`：GD25LQ128E 等片外器件驱动。
...
物理目录平铺不改变依赖方向：`*_stm32h5` 不得依赖 `board_*`，设备驱动不得直接读取 `board_config.h`，应用不得包含 STM32H5 私有头文件。
...
- `fatal`：主时钟、内存、必需安全输出或必需调试通道失败；进入定义好的安全状态并记录阶段/错误。
