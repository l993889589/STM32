...
`bsp_clock_stm32h5` 是 RCC、PLL 和系统总线时钟的唯一 owner。初始化顺序固定为：
...
- 记录 `fatal_stage` 和具体 clock error；
- 将受控输出保持安全状态；
...
- ThreadX 工程中的 TIM17 为 `bsp_timebase_stm32h5` 独占资源；
...
1. ICACHE/DCACHE 初始化由 `bsp_memory_stm32h5` 唯一负责；
...
进入 fatal handler 后不得关闭中断再调用依赖 tick 的 `HAL_Delay()`。故障指示使用独立有界周期或硬件计数延时，并保持所有安全输出状态。
