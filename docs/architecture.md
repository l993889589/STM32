...
D:/Embedded/H5
  ...
  docs/                    项目自写文档
  STM32H563_App/user/ldc/  App 本地 LDC 通信库
  STM32H563_Bootloader/user/ldc/ Bootloader 本地 LDC 通信库
  tests/                   主机侧测试
...
STM32 HAL / CMSIS / USBX / ThreadX
        -> BSP
        -> project-local communication libraries
...
- AT modules 负责具体模组差异，例如 W800、EC20。NearLink 已从当前应用移除，后续如恢复应作为独立模块重新接入。
...
- `app_console`：USART3 Console / Debug 输入链路。
...
- GPIO：
  ...
  - PB14/PB15：FT6336U touch reset/int。
...
- 触摸：`bsp_touch_init()` 只执行 reset 和状态清零；`touch status/read` shell 命令首次访问时再初始化 I2C1 并探测 FT6336U 地址 `0x38`，避免把 I2C 访问放到开机关键路径。
