...
{
    return g_spi != NULL && HAL_SPI_Transmit(g_spi, (uint8_t *)data, len, GD25_SPI_TIMEOUT_MS) == HAL_OK;
}

static bool gd25_rx(uint8_t *data, uint16_t len)
{
    return g_spi != NULL && HAL_SPI_Receive(g_spi, data, len, GD25_SPI_TIMEOUT_MS) == HAL_OK;
}
